...
func webauthnTestServer(t *testing.T) (*httptest.Server, *store.Store, *pgxpool.Pool) {
	...
	if _, err := pool.Exec(ctx, `TRUNCATE facts, fact_scopes, grants, grant_scopes, staged_diffs, audit_log, reviewer_tokens, agent_tokens, webauthn_credentials, webauthn_challenges, enrollment_latch, grant_requests, data_keys, capability_grant_identities, capability_grant_tokens, signing_policies`); err != nil {
