...
func (s *Store) ApproveGrantRequest(ctx context.Context, id, decidedBy string) (Grant, error) {
	...
	// Re-validated here, not just trusted from RequestGrant: requested_scopes
	...
	if err := logAudit(ctx, qtx, "grant_request_approved", decidedBy, nil, &grantRow.ID, nil, &id, req.RequestedScopes, nil); err != nil {
		return Grant{}, err
	}
...
func (s *Store) DenyGrantRequest(ctx context.Context, id, decidedBy string) error {
	...
	if err := logAudit(ctx, qtx, "grant_request_denied", decidedBy, nil, nil, nil, &id, nil, nil); err != nil {
		return err
	}
