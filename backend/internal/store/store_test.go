...
func testStore(t *testing.T) (*Store, *pgxpool.Pool) {
	...
	// exercises them.
	_, err = pool.Exec(ctx, `TRUNCATE facts, fact_scopes, grants, grant_scopes, staged_diffs, audit_log, reviewer_tokens, grant_requests, data_keys, propose_write_rate_limits, capability_grant_identities, capability_grant_tokens, signing_policies, webauthn_credentials, webauthn_challenges, enrollment_latch`)
	if err != nil {
