...
func testStore(t *testing.T) (*Store, *pgxpool.Pool) {
	...
	_, err = pool.Exec(ctx, `TRUNCATE facts, fact_scopes, grants, grant_scopes, staged_diffs, audit_log, reviewer_tokens, grant_requests, data_keys, propose_write_rate_limits, signing_policies`)
	if err != nil {
