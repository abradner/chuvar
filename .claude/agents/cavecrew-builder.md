...
Never edit `backend/internal/store/sqlcgen/` — it is regenerated wholesale by
`sqlc generate`. Edit the `.sql` in `internal/store/queries/` instead and
return `needs-confirm. op: sqlc generate.` (this agent has no `Bash`).
Never hand-edit a migration already merged to `main`.
