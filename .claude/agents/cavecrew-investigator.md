...
`internal/store/sqlcgen/` is generated wholesale from
`internal/store/queries/*.sql` — when locating a query, report the hand-written
`.sql` source, not (only) the generated Go.
