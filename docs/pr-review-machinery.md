...
> **Roster verified against this repo's own PR history on 2026-09-02** (`gh pr list -R abradner/chuvar
> --state all --limit 100 --json number,reviews`, 81 PRs): **Copilot** (`copilot-pull-request-reviewer`)
> has left 90 review records across 76 of those PRs, most recently on #130. **Codex**
> (`chatgpt-codex-connector`) has left 29 review records across 21 PRs, most recently on #119 — and
> on this repo every Codex review followed an explicit `@codex review` issue comment (see #116, #119),
> so Codex here is **on request only**, never automatic. Both rows below are live; re-verify rather
> than trust this note when it is more than a few weeks old. A steering doc in a sibling repo once
> named a reviewer whose app had never been installed; PRs satisfied the letter of the rule while
> getting one bot pass instead of two, and nobody noticed for weeks. When re-verifying, check
> **all three surfaces** from §2 — a bot that only leaves inline or issue comments never appears in
...
> ```
>
> Delete a row only if a re-check shows that reviewer has gone.
...
Evidence, from the shared template's own history (its PR number 5 — spelled out rather than
written as a bare "#5", which resolves to an unrelated PR in every downstream repo): the operator's go-ahead was followed by three further
...
The merge gate itself is a rule the shared template carries in its `AGENTS.md`; this repo's
`AGENTS.md` does not state it, and `batch-review`'s "Merge policy" line is the nearest text here —
**read that before merging anything.** The template's `AGENTS.md` is canonical for the gate, exactly
as this file is canonical for the mechanics above. The one-line
...
count — belongs with the gate and is deliberately not duplicated here.
...
because it runs in repos that have no template-derived `AGENTS.md` and no `docs/` — it is
regenerated from the shared template's `batch-review`, so changes here reach it by regeneration,
not by reference. That copy is
